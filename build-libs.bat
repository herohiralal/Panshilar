...
if exist "%LINUX_X64_TOOLCHAIN%\clang.exe" (
    ...
    "%LINUX_X64_TOOLCHAIN%\clang.exe" -c -std=c11 -DPNSLR_IMPLEMENTATION -DPNSLR_LINUX=1 -DPNSLR_X64=1 Source/zzzz_Unity.c -o Temp/unity-linux-x64.o --sysroot=Toolchains\Linux-x64\ %CLANG_WARNINGS% --target=x86_64-unknown-linux-gnu
    if errorlevel 1 (
...
if exist "%LINUX_ARM64_TOOLCHAIN%\clang.exe" (
    ...
    "%LINUX_ARM64_TOOLCHAIN%\clang.exe" -c -std=c11 -DPNSLR_IMPLEMENTATION -DPNSLR_LINUX=1 -DPNSLR_ARM64=1 Source/zzzz_Unity.c -o Temp/unity-linux-arm64.o --sysroot=Toolchains\Linux-ARM64\ %CLANG_WARNINGS% --target=aarch64-unknown-linux-gnu
    if errorlevel 1 (
...
if exist "%ANDROID_TOOLCHAIN%\clang.exe" (
    ...
    "%ANDROID_TOOLCHAIN%\clang.exe" --target=aarch64-linux-android28 -c -std=c11 -DPNSLR_IMPLEMENTATION -DPNSLR_ANDROID=1 -DPNSLR_ARM64=1 Source/zzzz_Unity.c -o Temp/unity-android-arm64.o --sysroot=Toolchains\Android\toolchains\llvm\prebuilt\windows-x86_64\sysroot\ %CLANG_WARNINGS%
    if errorlevel 1 (
