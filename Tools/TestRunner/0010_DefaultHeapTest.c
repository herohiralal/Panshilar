...
MAIN_TEST_FN(ctx)
{
    ...
    ArraySlice(u64)      data = PNSLR_MakeSlice(u64, longsCount, true, PNSLR_GetAllocator_DefaultHeap(), &err);
    ...
    PNSLR_ResizeSlice(u64, data, longsCount * 2, true, PNSLR_GetAllocator_DefaultHeap(), &err);
    if (!Assert(err == PNSLR_AllocatorError_None)) return;
    ...
    err = PNSLR_AllocatorError_None;
    PNSLR_FreeSlice(data, PNSLR_GetAllocator_DefaultHeap(), &err);
